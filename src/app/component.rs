...
pub trait Component {
    fn new(state: &State, action_tx: &tokio::sync::mpsc::UnboundedSender<Action>) -> Self
    where
        Self: Sized;
...
impl Component for InputBox {
    fn new(_state: &State, _action_tx: &tokio::sync::mpsc::UnboundedSender<Action>) -> Self {
        Self {
            //
            text: String::new(),
            cursor_position: 0,
    ...
    fn handle_key_event(&mut self, key: KeyEvent) {
        ...
        match key.code {
            KeyCode::Char(to_insert) if !contains_control => {
...
impl ComponentRender<RenderProps> for InputBox {
    fn render(&self, frame: &mut Frame, props: RenderProps) {
        let input = Paragraph::new(self.text.as_str())
            ...
            .block(Block::default().fg(props.border_color));
